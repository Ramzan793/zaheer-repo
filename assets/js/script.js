...
if ($('.typed').length > 0) {
	...
	document.addEventListener('DOMContentLoaded', function () {
		ityped.init(document.querySelector(".typed"), {
			strings: ['Building Construction', 'Plumbing & Heating', 'Electrical Service', 'Air Conditioning', 'Ventilation', 'Solar Panel', 'Security & Safety'],
...
// 	const dropdown = document.getElementById('multiple-dropdown');
// 	dropdown.style.display = dropdown.style.display === 'block' ? 'none' : 'block';
//   }

//   function applySelection() {
// 	const checkboxes = document.querySelectorAll('.multi-dropdown input[type="checkbox"]:checked');
// 	const selectedText = Array.from(checkboxes).map(cb => cb.value).join(', ');
// 	document.querySelector('.selected-items').innerText = selectedText || 'Select categories';
// 	document.getElementById('multiple-dropdown').style.display = 'none';
//   }
...
function updatePrice() {
	...
	document.getElementById('price-display').innerText = `Price: Rs. ${finalPrice.toFixed(2)}`;
}
...
document.querySelectorAll('input[name="cityScope"]').forEach(rb => {
	rb.addEventListener('change', (e) => {
		...
		document.getElementById('single-city-select').classList.add('hidden');
		document.getElementById('multi-city-select').classList.add('hidden');
		document.getElementById('area-select').classList.add('hidden');

		if (type === 'single') {
			document.getElementById('single-city-select').classList.remove('hidden');
			document.getElementById('area-select').classList.remove('hidden');
		} else if (type === 'multiple') {
			document.getElementById('multi-city-select').classList.remove('hidden');
		}
	});
});
